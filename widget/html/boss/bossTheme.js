define(function (require, exports, module) {
    ...
    /**
     * entry 1: memberIndex ---> here
     * entry 2: home ---> here
     */

    var vm = new Vue({
        el: '#bossTheme',
        template: _g.getTemplate('boss/bossTheme-body-V'),
        data: {
            /*-- 日常关怀 --*/
            title:'日常关怀',
            hasRecall: 20,
        ...
        methods: {
            ...
            //日常关怀
            onAllThemeV2: function (type, careId) {
                _g.openDrawerLayout({ 
                    header: {
                        data: {
                            title: '回访进度'
    ...
    var getSpecialData = function (opts,callback) {
        ...
        Http.ajax({
            ...
            success: function (res) {
                logger.log({"Type":"operation","action":"老板关怀主题详细获取","Win_name":api.winName,"data":_data,"message":res,"requireURL":_url})
                if (res.code == 200) {
    ...
    var getUsualData = function () {
        ...
        Http.ajax({
            ...
            url:_url,
            success: function (res) {
                logger.log({"Type":"operation","action":"老板日常关怀数据获取","Win_name":api.winName,"data":_data,"message":res,"requireURL":_url})
                if (res.code == 200) {
                    vm.title = res.object[0].title || '日常关怀';
                    vm.hasRecall = res.object[0].visit_count || 0;
                    vm.startTime = res.object[0].deploy_time || '';
                    vm.themeType = res.object[0].type;
