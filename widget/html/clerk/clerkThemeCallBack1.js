define(function (require, exports, module) {
	...
	var type = api.pageParam.type||'special';
	var Time = require('U/getTime');
	var Http = require('U/http');
	var vm = new Vue({
		el: '#clerkThemeCallBack1',
		template: _g.getTemplate('clerk/clerkThemeCallBack1-body-V'),
		data: {
			list: [
				{
					achieved_rate: '',//
	...
	var getData = function () {
		...
		var _data= {
				care_id: entId,//	关怀主题ID	number	日常主题没有主题ID entId
		...
		Http.ajax({
			...
			success: function (res) {
				logger.log({"Type":"operation","action":"店员获取关怀主题","Win_name":api.winName,"data":_data,"message":res,"requireURL":_url})
				if (res.code == 200) {
					console.log(res)
					vm.list = getItem(res);
				} else {
