...
//     var purchaseReturnList = new Vue({
//         el: '#purchaseReturnList',
//         template: _g.getTemplate('shop/purchaseReturnList-list-V'),
//         data: {
//             noData: false,
//             tapList: [{
//                 text: '未审核',
...
//                     page: page
//                 },
//                 url: '/app/auth/page/erp/erpReturnOrderList.do',
//                 success: function(ret) {
//                     if (ret.code == 200) {
//                         setTimeout(function() {
//                             if (ret.object) {
//                                 if (ret.object.lists.length < 1 && page == 1) {
//                                     purchaseReturnList.noData = true;
//                                     next = false;
//                                 } else if (ret.object.lists.length > 0 && page >= 1) {
//                                     purchaseReturnList.noData = false;
//                                     next = true;
//                                     var list = getDataList(ret.object);
//                                     if (purchaseReturnList.list) {
//                                         purchaseReturnList.list = purchaseReturnList.list.concat(list);
//                                     } else {
//                                         purchaseReturnList.list = list;
//                                     }
...
//             return {
//                 purchaseNo: item ? item.return_order_no : 0,
...
//                 orderId: item ? item.return_order_id : 0
//             }
//         });
//     }
...
define(function(require, exports, module) {
    ...
    var purchaseOrderList = new Vue({
        ...
        methods: {
            onDetailTap:function (orderId) {
                ...
                logger.log({"Type":"operation","action":"点击采购退货列表具体退货单","Win_name":api.winName,"data":_data})
                _g.openWin({
                    header: {
                        data: _data
                    },
    ...
    var getData = function (opts,callback) {
        ...
        var _url='/app/auth/erp/listReturnOrder.do';
        Http.ajax({
            data:_data,
            api_versions: 'v2',
            url:_url,
            success:function (res) {
                logger.log({"Type":"operation","action":"获取采购退货单列表","Win_name":api.winName,"data":_data,"message":res,"requireURL":_url})
                purchaseOrderList.isFirstLoading = false // 第一次加载判断
                if(res.success){
                    purchaseOrderList.isSearch = false; // 搜索按钮判断
    ...
    api.addEventListener({
        ...
        logger.log({"Type":"operation","action":"采购退货单列表接收头部发送信息，打开筛选侧边栏","Win_name":api.winName});
        api.openDrawerPane({
            type: 'right'
        });
    });
    ...
    api.addEventListener && api.addEventListener({
        ...
        logger.log({"Type":"operation","action":"接收搜索框信息，重新刷新采购退货单页面","Win_name":api.winName});
    ...
    api && api.addEventListener({
        ...
        logger.log({"Type":"operation","action":"返回，重新刷新采购退货单页面","Win_name":api.winName});
        getData();
        loadmore.reset();
    });
    module.exports = {};
