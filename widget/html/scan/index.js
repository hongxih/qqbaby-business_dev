define(function(require, exports, module) {
    ...
    var getGoodFlowForScan = function(content) {
        ...
        var _url='/app/auth/page/product/listForStockChangeLog.do';
        Http.ajax({
            data:_data,
            isSync:true,
            url:_url,
            success: function(ret) {
                 logger.log({"Type":"operation","action":"扫二码商品流水查询","Win_name":api.winName,"data":_data,"message":ret,"requireURL":_url})
                if (ret.code == 200) {
                    showScanner();
                    if (!ret.object) {
                        _g.toast('没有匹配的商品信息');
    ...
    function getGoodsCostCheck(content) {
        ...
        Http.ajax({
            ...
            url:_url,
            success: function(ret) {
                 logger.log({"Type":"operation","action":"库存成本查询扫二码","Win_name":api.winName,"data":_data,"message":ret,"requireURL":_url})
                if (ret.code == 200) {
                    showScanner();
                    if (!ret.object) {
                        _g.toast('没有匹配的商品信息');
    ...
    function getInventory(content) {
        ...
        var _url='/app/auth/page/product/listCheckOrder.do';
        Http.ajax({
            data:_data,
            isSync:true,
            url:_url,
            success: function(ret) {
                 logger.log({"Type":"operation","action":"库存查询盘点扫二码","Win_name":api.winName,"data":_data,"message":ret,"requireURL":_url})
                if (ret.code == 200) {
                    showScanner();
                    if (!ret.object) {
                        _g.toast('没有匹配的商品信息');
